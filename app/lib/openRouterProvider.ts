...
async function generateComponentTreeFromOpenRouter(
  ...
  try {
    ...
    try {
      ...
      if (dataLines.length === 0) {
        throw new Error("No valid data lines found in SSE response");
      }
      ...
      if (!fullContent) {
        throw new Error("No content found in SSE response");
      }
    ...
    throw new Error("Failed to generate component tree from OpenRouter API.");
  }
}

export class OpenRouterCodeGenerator {
  async create(
    request: AIGenerationRequest,
    model?: string
  ): Promise<{ code: string; components: ComponentNode[] }> {
    const aiResponse = await generateComponentTreeFromOpenRouter(
      request,
      model
    );
...
${spaces}</${Element}>`;
  ...
  private getDefaultModels(): Array<{
    id: string;
    name: string;
    description: string;
  }> {
...
export const openRouterCodeGen = new OpenRouterCodeGenerator();
