...
export class AICodeGenerator {
  async create(request: AIGenerationRequest): Promise<{ code: string; components: ComponentNode[] }> {
  ...
  private async generateComponents(request: AIGenerationRequest): Promise<ComponentNode[]> {
    ...
    if (prompt.toLowerCase().includes('chart') || prompt.toLowerCase().includes('visualization')) {
    ...
    return [{
      ...
      type: 'container',
      props: { className: 'p-6 bg-white rounded-lg shadow-md max-w-md mx-auto' },
      position: { x: 50, y: 50 },
  ...
  private createQuizComponents(): ComponentNode[] {
    return [{
      id: 'quiz-container',
      type: 'container',
      props: { className: 'p-6 bg-white rounded-lg shadow-lg max-w-2xl mx-auto' },
      position: { x: 50, y: 50 },
      ...
      children: [
        ...
        },
        {
          ...
          props: { 
            ...
            className: 'w-full text-left p-3 mb-2 border border-gray-300 rounded hover:bg-gray-50'
          },
          position: { x: 0, y: 140 },
    ...
    return [{
      ...
      children: [
        ...
        },
        {
          ...
          type: 'container',
          props: { className: 'w-full h-64 bg-gradient-to-r from-blue-100 to-purple-100 rounded-lg flex items-center justify-center' },
          position: { x: 0, y: 60 },
    ...
    return [{
      ...
      children: [
        ...
        },
        {
          id: 'basic-plan',
          type: 'container',
          props: { className: 'bg-white p-6 rounded-lg shadow-md border-2 border-gray-200' },
          position: { x: 20, y: 70 },
        ...
        },
        {
          id: 'pro-plan',
          type: 'container',
          props: { className: 'bg-white p-6 rounded-lg shadow-md border-2 border-blue-500' },
          position: { x: 280, y: 70 },
          size: { width: 240, height: 300 },
          styles: {},
          children: [{
  ...
  private generateReactCode(components: ComponentNode[], request: AIGenerationRequest): string {
...
export default GeneratedArtifact`
  ...
  private renderComponentCode(node: ComponentNode, indent: number = 0): string {
    ...
    const childrenCode = children?.map(child => 
      this.renderComponentCode(child, indent + 1)
    ).join('\n') || ''
